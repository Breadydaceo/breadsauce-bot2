...
def recharge_menu(call):
    ...
    for amt in amounts:
        kb.add(InlineKeyboardButton(f"💵 ${amt}", callback_data=f"recharge_usd_{amt}"))
    kb.add(InlineKeyboardButton("🔙 Back", callback_data="main_menu"))
    bot.edit_message_text("🪙 *Choose your recharge amount:*", call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
...
def create_invoice(call):
    ...
    if response.ok and "payment_redirection_url" in response.json():
        url = response.json()["payment_redirection_url"]
